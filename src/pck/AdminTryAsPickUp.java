 ...
 * @author intel
 */
public class AdminTryAsPickUp extends javax.swing.JFrame {
...
private void Update_table(){
    try{
        String sql = "select ID, Product_Name as \"Product Name\", Price, Qty as Quantity  from store";
        //String sql = ("Select * From Category Where id = '" + lb + "'  ");
        pst = Conn.prepareStatement(sql);
        rs = pst.executeQuery();
        //byte[]imagedata = rs.getBytes("image");
...
private void criticalProduct(){
    try{
        String sql = ("SELECT `ID`, `Product_Name` as \"Product Name\", `Price`,Qty as Quantity FROM store WHERE Qty <= '10'" );
        pst = Conn.prepareStatement(sql);
        rs = pst.executeQuery();
        //while(rs.next()){
        //rs.first();
...
public AdminTryAsPickUp(){
    initComponents();
    populateJTable();
    Conn = AdminTryAsPickUp.javaconnect.ConnecrDB();
...
//            PickupdesignViewProduct.productid.setText(rs.getString("ID"));
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(AdminTryAsPickUp.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new AdminTryAsPickUp().setVisible(true);
            }
        });
    }
